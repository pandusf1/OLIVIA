...
<head>
    ...
    <title>Detail Klien - {{ $client->name }}</title>
...
<main class="max-w-6xl mx-auto px-6 py-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-black text-gray-950">{{ $client->name }}</h1>
                <p class="text-sm text-gray-500">Detail Informasi Klien & Layanan yang Dipesan</p>
            </div>
        </div>
        <div>
            <a href="{{ route('chat.messages', ['mitraId' => $client->id]) }}" class="inline-flex items-center gap-2 bg-gray-950 hover:bg-black text-white font-bold px-6 py-3 rounded-xl transition shadow-sm text-sm">
        ...
        <div class="lg:col-span-7 space-y-6">
            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                        ...
                        <span class="md:col-span-2 text-sm font-semibold text-gray-900">{{ $client->name }}</span>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                        <span class="text-xs font-bold uppercase tracking-wider text-gray-400">Email</span>
                        <span class="md:col-span-2 text-sm font-semibold text-gray-900">{{ $client->email }}</span>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                        <span class="text-xs font-bold uppercase tracking-wider text-gray-400">No. WhatsApp / HP</span>
                        <span class="md:col-span-2 text-sm font-semibold text-gray-900">{{ $client->phone ?: '-' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                ...
                <div class="divide-y divide-gray-100">
                    @forelse($purchasedServices as $ps)
                        <div class="py-4 first:pt-0 last:pb-0">
                            <div class="flex justify-between items-start gap-4">
                                ...
                                <div class="text-right">
                                    <p class="font-bold text-gray-900 text-sm">Rp {{ number_format($ps->priceList?->price ?? 0, 0, ',', '.') }}</p>
                                    <span class="inline-block mt-2 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider {{ $ps->status === 'paid' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800' }}">
                                        {{ $ps->status === 'paid' ? 'Sudah Bayar' : 'Negosiasi' }}
                                    </span>
                                </div>
                            </div>
                        </div>
