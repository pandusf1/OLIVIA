    ...
    <div id="step-detail" class="fade-in">
        ...
        <div class="flex items-center gap-3 mb-6">
            <div class="w-10 h-10 rounded-xl bg-gray-100 border border-gray-200 overflow-hidden flex-shrink-0">
                @if(!empty($priceList->partner->image_url))
                    <img src="{{ $priceList->partner->image_url }}" class="w-full h-full object-cover"/>
                @else
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-5 mb-4">
            ...
            <div class="flex justify-between items-start gap-4 py-3 border-b border-gray-100">
                ...
                <p class="font-black text-gray-900 shrink-0">Rp {{ number_format($priceList->price, 0, ',', '.') }}</p>
            </div>

            <div class="flex justify-between items-center py-3">
                <p class="text-sm text-gray-500">Total</p>
                <p class="font-black text-xl text-gray-900">Rp {{ number_format($priceList->price, 0, ',', '.') }}</p>
            </div>
        </div>
    ...
    <div id="step-success" class="hidden flex flex-col items-center py-16 text-center fade-in">
        ...
        <div class="w-full bg-green-50 border border-green-200 rounded-2xl px-5 py-4 mb-6 text-left">
            ...
            <div class="flex justify-between items-center text-sm">
                ...
                <span class="font-black text-gray-900">Rp {{ number_format($priceList->price, 0, ',', '.') }}</span>
            </div>
        </div>

        <a id="btn-to-chat" href="{{ route('chat.start', ['partnerId' => $priceList->partner_id]) }}"
...
function startPayment() {
    ...
    setTimeout(function() {
        ...
        fetch("{{ route('pembayaran.pay') }}", {
            ...
            body: JSON.stringify({ price_list_id: {{ $priceList->id }} })
